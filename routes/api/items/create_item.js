...
router.post("/", isAuthorized(["admin"]), async (req, res) => {
    ...
    if (tags) {
        ...
        const [tagResults] = await connection.query(`SELECT id FROM Tags WHERE name IN (${tagPlaceholders});`, tags);
    ...
    try {
        ...
        const [result] = await connection.query("INSERT INTO Items (name, description, price, image_url, is_available) VALUES (?, ?, ?, ?, TRUE);", [name, description, price, imageUrl]);
        const itemId = result.insertId;
        if (tagIds.length > 0) {
            const itemTag = tagIds.map(tagId => [itemId, tagId]);
            const itemTagPlaceholders = itemTag.map(() => '(?, ?)').join(', ');
            const query = `INSERT INTO ItemTags (item_id, tag_id) VALUES ${itemTagPlaceholders};`;
